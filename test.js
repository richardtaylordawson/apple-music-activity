...
class Computation {
  ...
  static isSamePlay(play, previousPlay) {
    if (
      ...
      previousPlay["End Position In Milliseconds"] ===
        play["Start Position In Milliseconds"] &&
  ...
  static isSamePlayNext(play, nextPlay) {
    if (
      ...
      play["End Position In Milliseconds"] ===
        nextPlay["Start Position In Milliseconds"] &&
  ...
  static isPlay(play) {
    if (
      varExists(play["Song Name"]) &&
      play["Song Name"].length > 0 &&
  ...
  static calculateTop(data, excludedSongs, callback) {
    ...
    for (let index = 0; index < data.length; index++) {
      ...
      if (
        varExists(play["Song Name"]) &&
        varExists(play["Artist Name"]) &&
        varExists(play["Play Duration Milliseconds"]) &&
        varExists(play["Media Duration In Milliseconds"]) &&
        varExists(play["Event End Timestamp"]) &&
        ...
        if (Computation.isPlay(play)) {
          const uniqueID =
            "'" + play["Song Name"] + "' by " + play["Artist Name"];

          if (
            Number(play["Play Duration Milliseconds"]) > 8000 &&
            (play["Event Type"] === "PLAY_END" || play["Event Type"] === "")
          ) {
            ...
            var missedMilliseconds =
              Number(play["Media Duration In Milliseconds"]) -
              Number(play["Play Duration Milliseconds"]);
            ...
            songs[uniqueID].time =
              Number(songs[uniqueID].time) +
              Number(play["Play Duration Milliseconds"]);
            songs[uniqueID].missedTime =
              Number(songs[uniqueID].missedTime) + missedMilliseconds;
            ...
            if (!songs[uniqueID].excluded) {
              ...
              if (!Computation.isSamePlay(play, previousPlay)) {
                ...
                artists[play["Artist Name"]].plays =
                  artists[play["Artist Name"]].plays + 1;
              }

              totals.totalTime =
                Number(totals.totalTime) +
                Number(play["Play Duration Milliseconds"]);
              artists[play["Artist Name"]].time =
                Number(artists[play["Artist Name"]].time) +
                Number(play["Play Duration Milliseconds"]);
              artists[play["Artist Name"]].missedTime =
                Number(artists[play["Artist Name"]].missedTime) +
                missedMilliseconds;
              ...
              var dayID =
                date.getDate() +
                " " +
                Computation.monthNames[date.getMonth()] +
                ", " +
                date.getFullYear();
              ...
              if (
                ...
                dayint > 0 &&
                varExists(hoursint) &&
                varExists(heatmapData[dayint][hoursint]) &&
                varExists(Number(heatmapData[dayint][hoursint])) &&
                !isNaN(Number(heatmapData[dayint][hoursint])) &&
                !isNaN(Number(play["Play Duration Milliseconds"]))
              ) {
                heatmapData[dayint][hoursint] =
                  Number(heatmapData[dayint][hoursint]) +
                  Number(play["Play Duration Milliseconds"]);
              }
              ...
              months[monthID].time =
                Number(months[monthID].time) +
                Number(play["Play Duration Milliseconds"]);
              months[monthID].missedTime =
                Number(months[monthID].missedTime) + missedMilliseconds;
              ...
              if (!Computation.isSamePlay(play, previousPlay)) {
                yearSongs[yearID][uniqueID].plays =
                  yearSongs[yearID][uniqueID].plays + 1;
              }
              yearSongs[yearID][uniqueID].time =
                Number(yearSongs[yearID][uniqueID].time) +
                Number(play["Play Duration Milliseconds"]);
              yearSongs[yearID][uniqueID].missedTime =
                Number(yearSongs[yearID][uniqueID].missedTime) +
                missedMilliseconds;
              ...
              if (today === yearID) {
                ...
                if (!Computation.isSamePlay(play, previousPlay)) {
                  ...
                  thisYear.artists[play["Artist Name"]].plays =
                    thisYear.artists[play["Artist Name"]].plays + 1;
                }

                thisYear.totalTime =
                  Number(thisYear.totalTime) +
                  Number(play["Play Duration Milliseconds"]);
                thisYear.artists[play["Artist Name"]].time =
                  Number(thisYear.artists[play["Artist Name"]].time) +
                  Number(play["Play Duration Milliseconds"]);
                thisYear.artists[play["Artist Name"]].missedTime =
                  Number(thisYear.artists[play["Artist Name"]].missedTime) +
                  missedMilliseconds;
              }
            }
          }
    ...
    for (let index = 0; index < yearresult.length; index++) {
      yearresult[index].value = Computation.convertObjectToArray(
        yearresult[index].value
      );
    ...
    var thisYearArtsistsResult = Computation.convertObjectToArray(
      thisYear.artists
    );
